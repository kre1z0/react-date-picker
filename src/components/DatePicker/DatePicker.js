...
class DatePicker extends Component {
  static propTypes = {
    ...
    startDate: PropTypes.oneOfType([PropTypes.number, PropTypes.string, PropTypes.object]),
    endDate: PropTypes.oneOfType([PropTypes.number, PropTypes.string, PropTypes.object]),
  ...
  componentDidMount() {
    ...
    if (this.dateIsValid(value)) {
      this.setState({
        error: false,
        time: this._convertTimeToFilter(moment(value) - moment(value).startOf("day"), time),
  ...
  _getValues = value => {
    ...
    const time = this._convertTimeToFilter(moment(value) - moment(value).startOf("day"));
  ...
  _dateFormatting = (value, withTime) => {
    if (withTime === "fullDate") return moment(value).format(dateFormatWithTime);
    else return moment(value).format(dateFormat);
  };

  onChangeNumberInput = (e, key) => {
    const targetValue = Number(e.target.value);
    if (!this._isNumeric(targetValue) || TIME_LIMIT[key] < targetValue) return;
  ...
  onPickDate = val => {
    ...
    if (this.dateIsValid(val)) {
      ...
      if (withTime) {
        value = new Date(moment(val).startOf("day") + this._convertFiltersToTime(this.state.time));
      }
  ...
  render() {
    ...
    return (
      ...
      >
        <Portal>
          <div
            ...
            style={{
              ...
              top: offsetTop,
              left: offsetLeft,
              transform: `translateY(${position === "top" ? -100 : height}${
                position === "top" ? "%" : "px"
              })`,
            }}
          >
            ...
            <Calendar
              ...
              date={date}
            />
            {withTime && <Time time={time} onChange={this.onChangeNumberInput} />}
          </div>
        </Portal>
        <IMaskInput
        ...
        />
        {error && <div className="datePicker-error">неверный формат даты</div>}
      </div>
    );
  }
}
